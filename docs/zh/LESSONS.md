...
## 打印「成功」和「退出 0」是两件事 —— 而且这条静态查不出来

2026-09-02，CI 上编 14 个工具的循环写着 `tools/build-$t.sh || exit 1`，红在
`fastboot`。日志里 fastboot **三条验收全绿、还打了「这份 fastboot 是自己编的，
可以用了」**，紧接着就是 `Error: Process completed with exit code 1`。
...
common_verify_done() {
  ...
    [ -n "${1:-}" ] && echo "$1"     # 参数为空 -> 求值为假 -> 函数返回 1
  fi
}                                     # 而它是各 build-*.sh 的最后一条命令
```

传空参数的有 `fastboot`、`simpleperf`，以及 `aapt2`/`dexdump`/`split-select`
的部分分支。**藏了很久**，因为单独跑脚本时人只看输出、不看 `$?`。这个仓库最
强调的就是「退出码是契约」，自己先违反了。

想把它变成机器守卫，试了两版静态检查，都不成立：

| 判据 | 结果 |
|---|---|
| 「函数最后一行是裸的 `[ … ] &&`」 | **抓不住**：真正咬人的那次最后一行是 `fi`，条件式在 if 分支里 |
| 放宽成「是所在块的最后一句」 | 开始**误报** `for` 循环体里的正常写法（循环后面还有代码，返回值不受影响） |

「函数返回前最后执行的是哪条语句」要真的控制流分析，不是 grep 能干的。所以
**别硬做静态检查**，改成真跑一遍：`tools/check-exit-codes.sh` 跑每个脚本的
`--verify` 只看退出码，14 个全过；变异测试（把 `return 0` 去掉）确认它会红。

**一条抓不住自己动机的检查，比没有更糟** —— 它会让人以为这类问题已经有人看着。
